...
public:
	...
	*/
	...
	//return a pointer to the starting square
	Square* getStart() const;

	//to search for other letters around places tiles
	std::pair<std::string,unsigned int> search(Tile* current, size_t x_pos, size_t pos_y, bool direction)const;

	//to search for letters above and below the word being placed
	std::pair<std::string,unsigned int> original_search(std::vector<Tile*> original, size_t x_pos, size_t y_pos, bool direction) const;

	//to initialize tiles if given an initialization file
	void init_board(std::string init_file);

	//returns whether an init file was used when creating board
	bool initFileUsed();
